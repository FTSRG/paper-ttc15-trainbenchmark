...
\begin{itemize}
  ...
  \item For each size $\mathit{s}$ from 1 up to $\mathit{max}$, if a tool is able to complete the validation, it is rewarded points proportional to the size of the target model, i.e.\
  	\begin{itemize}
  	  \item round 1: the winner for $s=1$ earns 1 point,
  	  \item round 2: the winner for $s=2$ earns 2 points,
  	  \item round 3: the winner for $s=3$ earns 3 points,
  	  \item and so on.
  	\end{itemize}
  In total, a solution earns up to $1 + 2 + 3 + \ldots + max = \mathit{max}\cdot(\mathit{max}+1)/2$ points in a ranking. This results is normalized to 1, therefore the score for a size $s$ is:
